...
var Builder = (function () {
    ...
    Builder.prototype.performDiagnosticChecks = function () {
        console.log("Generating Diagnostics...");
        console.log();
        var compilerDiagnostics = this.services.getCompilerOptionsDiagnostics();
        assert(compilerDiagnostics.length == 0, compilerDiagnostics.length > 0 ? compilerDiagnostics[0].messageText.toString() : "");
        var program = ts.createProgram(this.tsConfig.fileNames, this.tsConfig.options);
        var preEmitDiagnostics = ts.getPreEmitDiagnostics(program);
        preEmitDiagnostics.forEach(function (diagnostic) {
            var prefix = ts.DiagnosticCategory[diagnostic.category] + " (" + diagnostic.code + "):";
            var prefixStyle = colors.yellow;
            if (diagnostic.category == ts.DiagnosticCategory.Error) {
                prefixStyle = colors.red;
            }
            var lineMap = diagnostic.file.lineMap;
            var line = 0;
            while (lineMap[line] <= diagnostic.start) {
                line++;
            }
            var col = diagnostic.start - lineMap[line - 1] + 1;
    ...
    Builder.prototype.buildAllFiles = function () {
        var _this = this;
        this.tsConfig.fileNames.forEach(function (fileName, index) {
            emit(_this.dir, _this.options, _this.tsConfig, _this.services, fileName, index);
        });
    };
...
function emit(dir, options, tsConfig, services, fileName, index) {
    var curFileCounter = (index !== undefined) ? " (" + (index + 1) + " of " + tsConfig.fileNames.length + ")" : '';
