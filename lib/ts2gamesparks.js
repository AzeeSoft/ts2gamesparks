...
function buildFile(tsConfig, services, filePath, index) {
...
console.log();
var compilerDiagnostics = services.getCompilerOptionsDiagnostics();
assert(compilerDiagnostics.length == 0, compilerDiagnostics.length > 0 ? compilerDiagnostics[0].messageText.toString() : "");
var program = ts.createProgram(tsConfig.fileNames, tsConfig.options);
...
console.log();
tsConfig.fileNames.forEach(function (file, index) {
